...
# Setting all figure text to size 12
...
def integrator(V, z_max=None, update_init=None, init_recent=None, upper_limit=None):
    ...
    if V == "power-law":
        if update_init == False: # if we want to plot z between 0 and 2
...
def Hubble_param(V, z_max=None):
    ...
    if z_max != None:
        ...
        init_recent = integrator(V, update_init=True, upper_limit=z_max)[4] # retrieving new initial conditions
...
def Hubble_lamdaCDM(omega_m0=None, z_max=None):
    ...
    if z_max != None:
        # For when we want z between 0 and 2 we need values in this range:
        init_recent = integrator("exp", update_init=True, upper_limit=z_max)[4] # retrieving new initial conditions
...
def age(V):
    """
    Function for calulating the dimensionless age H_0 t_0 of the Universe for the two
    ...
    print(f"Age of the universe t_0 = {t0[-1]} for V = {V}")
...
def luminosity_dist(model, omega_m0=None, z_max=None):
    ...
    init_recent = integrator(model, update_init=True)[4]
    omegas, w_phi, N, z, init_recent = integrator(model, z_max, update_init=False, init_recent=init_recent)
        
    integrand = np.exp(-N) / h
    #integrandz = 1 / h

    #integral_z = cumulative_trapezoid(np.flip(integrandz), np.flip(z), initial=0)
    integral_N = cumulative_trapezoid(np.flip(integrand), N, initial=0)

    #H0_d_L_over_c = (1 + z) * np.flip(integral_z)
...
def chi_square(model, omega_m0=None):
    ...
    if model == "exp" or "power-law":
        z_max = 2 
    if model == "lCDM":
        z_max == 2  # z_data[-1]

    if omega_m0 != None:
        d_L_model, z = luminosity_dist(model, omega_m0, z_max) # should have z go from 0 -> max(z_data)???
    else:
        d_L_model, z = luminosity_dist(model, z_max) # should have z go from 0 -> max(z_data)???
...
def omega_m0_best_fit(plot=None):
    ...
    possible_omega_m0 = np.linspace(0, 1, 100) # Could have had more points here
    chi2 = np.zeros(100)
    ...
    if plot == True:
        ...
        plt.ylabel("$\chi^2$")
        plt.axvline(best_m0, ymax=0.95, ymin=0.02, linestyle="--")
...
def plot_d_L(chi2=None):

    plt.figure(figsize=(10,5))

    if chi2 == True:
        data = chi_square("exp")[0]
