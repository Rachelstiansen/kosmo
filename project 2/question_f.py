...
# if __name__ == " __main__ ": # only if i change path
...
for i, y in enumerate(bbn.Y_i):
    ...
    ax.loglog(bbn.T, bbn.get_np_equil(bbn.T)[0] * bbn.mass_number[i], ls="dotted", color="orange")
    ax.loglog(bbn.T, bbn.get_np_equil(bbn.T)[1] * bbn.mass_number[i], ls="dotted", color="blue")
